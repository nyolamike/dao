defmodule DaoEngine do
  ...
  def gen_sql_ensure_table_exists(context, table_name, query_config) do
    ...
    if context.schema |> Map.has_key?(key) == false do
      ...
      IO.inspect("set_default_standard_pk")
      IO.inspect(key)
      IO.inspect(query_config)
      ...
      table_col_def =
        if Map.has_key?(query_config, :columns) == true do
          ...
          Enum.reduce(query_config.columns, acc, fn {column_name_key, column_config},
                                                    %{sql: sql_acc, table_schema: table_schema} ->
      ...
      table_schema =
        ...
        |> Map.put(:is_deleted_col, is_deleted_col_def.config)
        |> Map.put(:deleted_on_col, deleted_on_col_def.config)

      schema = Map.put(context.schema, key, table_schema)
