defmodule GetQueryTest do
  ...
  defp get_expected_results() do
    %{
      ...
      root_cmd_node_list: [
        get: [biggest_aircraft: [aircraft: %{is_list: false, sql: "SELECT * FROM `grocerify.aircraft` WHERE is_deleted == 0"}]],
        get: [list_of_shops: [shops: %{is_list: true, sql: "SELECT * FROM `grocerify.shops` WHERE is_deleted == 0"}]]
      ]
    }
  end
