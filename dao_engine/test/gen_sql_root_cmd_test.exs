defmodule GenSqlRootCmdTest do
  ...
  defp get_expected_results() do
    %{
      ...
      input_node_list: [list_of_shops: [shops: %{is_list: true, sql: "SELECT * FROM `grocerify.shops` WHERE is_deleted == 0"}]]
    }
  end
  ...
  defp get_expected_results_air_craft() do
    %{
      ...
      input_node_list: [
        biggest_aircraft: [aircraft: %{is_list: false, sql: "SELECT * FROM `grocerify.aircraft` WHERE is_deleted == 0"}]
      ]
    }
  end
end
